      ...
      {isActive && (
        <div className='w-full absolute left-0 top-12 bg-white text-black p-2 rounded-lg'>
          ...
          {searchValue && foundBooks.length ? (
            foundBooks.map((book) => <SearchItem key={book.id} {...book} onClose={() => setIsActive(false)} />)
          ) : searchValue && !foundBooks.length ? (
...
const SearchItem = ({ id, name, author, img, rating, onClose }) => {
  return (
    <Link to={`/books/${id}`}>
      <div className='flex justify-between hover:bg-gray-100 rounded-lg p-2 cursor-pointer' onClick={onClose}>
        <div className='flex'>
